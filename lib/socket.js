...
module.exports = function(codebox, socket, workspace) {
    ...
    var shellEmit = function(eventId, shellId) {
    ...
    shx.on('exit', id => _.partial(shellEmit, 'shell.exit'));
    shx.on('spawn', id => _.partial(shellEmit, 'shell.spawn'));
    shx.on('attach', id => _.partial(shellEmit, 'shell.attach'));
    shx.on('detach', id => _.partial(shellEmit, 'shell.detach'));
};
